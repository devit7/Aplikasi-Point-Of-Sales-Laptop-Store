<div>
    <button data-modal-target="delete-modal{{ $id }}" data-modal-toggle="delete-modal{{ $id }}"
        class="bg-[#002D4C] border p-1 border-[#2B4F69] rounded-md">
        {{-- @dd($type) --}}
        @if ($type == 'xButton')
            <svg fill="#ff0000" class="w-6 h-6" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg"
    ...
    <div id="delete-modal{{ $id }}" aria-hidden="true"
        class="mdl hidden overflow-x-hidden overflow-y-auto fixed h-modal md:h-full top-4 left-0 right-0 md:inset-0 z-50 justify-center items-center">
        <div class="flex w-[500px]  items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div
                ...
                <div class="sm:flex sm:items-start p-5">
                    <div
                        class="mx-auto flex-shrink-0 flex items-center justify-center w-12 h-12 rounded-full bg-red-100 sm:mx-0 sm:h-10 sm:w-10">
                ...
                <div class="flex justify-between items-center space-x-4 p-4 rounded-b">
                    <button data-modal-toggle="delete-modal{{ $id }}" type="submit"
                    ...
                    <form action="{{ route($route, $id) }}" method="POST">
                        ...
                        @method('delete')
                        <button type="submit"
                            class="py-2 px-4 text-sm rounded-md font-medium text-center text-white bg-red-600  hover:bg-red-700 focus:ring-4 focus:outline-none focus:ring-red-300 dark:bg-red-500 dark:hover:bg-red-600 dark:focus:ring-red-900">
                            Delete
                        </button>
                    </form>
                </div>
