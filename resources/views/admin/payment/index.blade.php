                    ...
                    @foreach ($data as $index => $payment)
                        <tr class="border-b border-[#33356F]">
                            ...
                            <td class="px-4 flex items-center gap-2">
                                <a href="{{ route('payment.edit', $payment['id']) }}" class="bg-blue-600 hover:bg-blue-700 text-white p-2 rounded-md">
                                ...
                                <form id="form-delete-payment-{{ $payment['id'] }}" action="{{ route('payment.destroy', $payment['id']) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button" onclick="confirmDelete('{{ $payment['id'] }}', '{{ $payment['payment_name'] }}')" class="bg-red-600 hover:bg-red-700 text-white p-2 rounded-md">
