                    ...
                    @foreach ($data as $index => $merk)
                        <tr class="border-b border-[#33356F]">
                            ...
                            <td class="px-4 flex items-center gap-2">
                                ...
                                <button onclick="confirmDelete('{{ $merk['id'] }}', '{{ $merk['merk_name'] }}')"
                                ...
                                <form id="form-delete-merk-{{ $merk['id'] }}" method="POST"
                                    action="{{ route('merk.destroy', $merk['id']) }}" class="hidden">
                                    @csrf
                                    @method('DELETE')
                                </form>
                            </td>
        ...
        <div class="flex items-center justify-center min-h-screen">
            <div class="bg-[#131432] rounded-lg p-6">
                <div class="mb-4 text-center">
                    <h1 class="text-white font-semibold">Apakah anda ingin menghapus opsi merk ini?</h1>
                </div>
                <div class="flex justify-center">
                    <button onclick="deletemerk()" class="px-4 py-2 mr-2 bg-red-600 text-white rounded-md">Iya</button>
    ...
    <script>
        ...
        function deletemerk() {
            if (merkIdToDelete) {
                ...
                document.getElementById('form-delete-merk-' + merkIdToDelete).submit();
            }
        }
